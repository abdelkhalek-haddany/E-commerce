...
use App\Traits\ImgMultiUpTrait;
...
use ImgMultiUpTrait;
    ...
    public function store(StoreProductRequest $request)
    {
        ...
        try {
            ...
            $imagesName = $this->SaveImage($request->images,'uploads/products');
                ...
                $product->save();
                return redirect()->route('v_products.index')->with(['success' => __('vendor/messages.saved')]);

        } catch (\Exception $ex) {
            return $ex;
            return redirect()->route('v_products.index')->with(['error' => __('vendor/messages.error')]);
        }
    }
    ...
    public function edit($id)
    {
        ...
        try {
            if (!$product)
                return redirect()->route('v_products.index')->with(['error' => __('vendor/messages.deleted')]);
            $categories = Category::get();
    ...
    {
        try {
            $product = Product::where('vendor_id',Auth::id())->get()->find($id);
            if (!$product)
                return redirect()->route('v_products.index')->with(['error' => __('vendor/messages.error')]);
            ...
            DB::commit();
            return redirect()->route('v_products.index')->with(['success' => __('vendor/messages.updated')]);
        } catch (\Exception $exception) {
            return $exception;
            DB::rollback();
            return redirect()->route('v_products.index')->with(['error' => __('vendor/messages.error')]);
    ...
    {
        try {
            $product = Product::where('vendor_id',Auth::id())->find($id);
            if (!$product)
                return redirect()->route('v_products.index')->with(['error' => __('vendor/messages.not_found')]);

           $status =  $product->is_published  == 0 ? '1' : '0';

           $product -> update(['is_published' =>$status ]);

            return redirect()->route('v_products.index')->with(['success' => __('vendor/messages.status_changed')]);
    ...
    try {
            ...
            return redirect()->route('v_products.index')->with(['success' => __('vendor/messages.deleted')]);
