...
@section('content')
    ...
    <main id="main">
        ...
        <div class="container" data-aos="fade-up">
            <h1 class="p-title">
                {{__("pages/blog/posts.page_title")}}
            </h1>
            <h4 class="p-desc">{{__("pages/blog/posts.page_desc")}}</h4>
        <div class="row">
        ...
        <div class="blog-pagination">
            ...
            <ul class="pagenation">
                @if ($posts->onFirstPage())
                    <li class="disabled"><span>{{__("pages/blog/globale.previous")}}</span></li>
                @else
                    <li><a href="{{ $posts->previousPageUrl() }}" rel="prev">{{__("pages/blog/globale.previous")}}</a></li>
                @endif
                @foreach ($posts as $post)
                    @if (is_string($post))
                ...
                <li><a href="{{ $posts->nextPageUrl() }}" rel="next">{{__("pages/blog/globale.next")}}</a></li>
            @else
                <li class="disabled"><span>{{__("pages/blog/globale.next")}}</span></li>
            @endif
        </ul>
    @endif 
        </div>
        @endif
            ...
            <div class="sidebar">
                {{--  End Search field --}}
                <h3 class="sidebar-title">{{__("pages/blog/globale.search")}}</h3>
                <hr>
                <div class="sidebar-item search-form">
                <form action="{{route('search')}}">
                    @csrf
                    <input type="text"  name="s" id="s" placeholder="{{__('pages/blog/globale.search_for')}}">
                    <button type="submit"><i class="fas fa-search"></i></button>
                </form>
                <br>
                <div class="latest-posts">
                    <h3 class="sidebar-title">{{__("pages/blog/globale.recent_posts")}}</h3>
                ...
                <div class="sidebar">
                <div class="tags">
                    <h3 class="sidebar-title">{{__("pages/blog/globale.tags")}}</h3>
                    <hr>
                    @foreach($tags as $tag)
