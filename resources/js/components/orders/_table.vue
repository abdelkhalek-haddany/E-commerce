        ...
        <tbody>
          <template v-for="order in orders">
            <tr v-bind:key="order.order.id" :class="'status'+order.order.confirmation+order.order.delivery">
              ...
              <td><a @click="deleteOrder(order.order.id)"
                  class="btn btn-outline-danger btn-min-width box-shadow-3 mr-1 mb-1">Delete</a></td>
            </tr>
          </template>
        </tbody>
      </table>
...
export default {
  ...
  methods: {
    ...
    deleteOrder(orderID) {
       this.loading = true;
      axios.get("/api/orders/destroy/"+orderID).then((response) => {
        this.loadOrders();
      })
      .catch(error => {
        this.loading = false;
        showMessage('this order not exists',0);
      });
      this.loading = false;
    },
  },
